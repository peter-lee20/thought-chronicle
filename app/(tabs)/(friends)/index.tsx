...
export default function FriendsPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        ); // Sort after all async operations
      } catch (error) {
  ...
  const findAndDelete = (list: Array<string>, friend: string) => {
    ...
    if (index !== -1) {
      // This check should never fail in normal app use
      list.splice(index, 1);
    } else {
  ...
  const removeFriend = async (friendId: string) => {
    ...
    const userQuery = query(userCollection, where("userId", "in", ids));
                  ...
                  onPress: async () => {
                    ...
                    setSentRequests((prevSentRequests) => [
                      ...prevSentRequests,
                      {
                        id: friendId,
                        username: friendName,
                        name: `${friendDocument.data().firstname} ${
                          friendDocument.data().lastname
                        }`,
                        buttons: [{ label: "Pending", onPress: () => {} }], // Or any other relevant action
                      },
                    ]);
